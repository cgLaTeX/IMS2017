...
Our solution to the coupled system proceeds via
  ...
  \end{inparaenum}
We employ a highly-tuned predictor-corrector integration scheme to advance the source wavefunctions in time; the polarizations that arise then serve as point-like sources within the time domain integral equations electromagnetic integral equations that we use to propagate the field.
...
Current full-wave solution strategies often resort to a finite-difference strategy for propagating fields throughout the simulation domain---a technique plagued by significant computational overhead and numerical sensitivities.
