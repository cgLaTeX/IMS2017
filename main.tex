...
The nonlinearities inherent in coupling \cref{eq:liouville} between dots places significant constraints on $\Delta t$, however, as one must choose $\Delta r_\text{min} > c \Delta t$ to
ensure a self consistent solution to the non-linear system; a condition that results in an infeasible number of time steps for an accurate solution.
