...
\title{\LARGE \bf
Towards a Self-Consistent Integral Equation Formulation of Optically-Active Media
}
...
\author{Connor Glosser$^{1,2}$, B.\ Shanker$^{2}$, and Carlo Piermarocchi$^{1}$% <-this % stops a space
\thanks{$^{1}$Michigan State University Department of Physics \& Astronomy, East Lansing, MI 48824, USA}%
\thanks{$^{2}$Michigan State University Department of Electrical \& Computer Engineering, East Lansing, MI 48824, USA}%
}
...
Conventional models of electromagnetic systems give rise to interesting phenomena through prescription of static boundary conditions.
  ...
  These processes have proven essential in countless technological applications motivating the need for an efficient fullwave Maxwell solver that couples to an underlying description of the system behavior.
  ...
  \begin{inparaenum}[(1)]
    \item determination of source polarizations through evolution of the differential optical Bloch equations, and
    \item evaluation of radiation patterns through well-known CEM techniques.
  \end{inparaenum}
  We employ a highly-tuned predictor-corrector integration scheme to advance (1) in time and the subsequent polarizations serve as pointlike sources to electromagnetic integral equations (chosen to facilitate accurate point-to-point communication of fields without the computational overhead of a ``radiation grid'').
